...
const TransactionTable = ({ transactions, plan = "anonymous", accountHolder, dateRange }: TransactionTableProps) => {
  ...
  const exportCSV = () => {
    const header = "Date,Time,Description,Payment Type,Account Name,Transaction ID,Direction,Amount (UGX),Fees (UGX),Tax (UGX),Running Balance (UGX),Category,Reference\n";
    ...
    const rows = filtered
      .map((t) => {
        ...
        return `"${displayDate}","${time}","${t.description}","${t.transactionType}","${t.accountName || ""}","${t.transactionId}","${direction}",${t.amount},${t.fees},${t.taxes},${t.balance},"${t.category}","${t.reference || ""}"`;
      })
      .join("\n");
    ...
    const dateFrom = formatDateDisplay(dateRange?.from || "").replace(/\s+/g, "");
    const dateTo = formatDateDisplay(dateRange?.to || "").replace(/\s+/g, "");
